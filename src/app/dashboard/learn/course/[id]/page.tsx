...
export default function CourseDetailPage({ params }: { params: Params }) {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      const courseRes = await supabase.from('courses').select('id,title,owner_id').eq('id', params.id).maybeSingle();
      if (courseRes.data && !courseRes.error) {
        setTitle(courseRes.data.title as string);
        setOwnerId(courseRes.data.owner_id as string);
      } else if (meta) {
        setTitle(meta.title);
      }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {user && ownerId === user.id && (
          <Link
            href={`/dashboard/learn/course/${params.id}/lesson/new`}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {(lessons ?? meta.lessons).map((ls) => (
          <Link
            key={ls.slug}
            href={`/dashboard/learn/course/${params.id}/lesson/${ls.slug}`}
